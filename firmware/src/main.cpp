...
// Raw timing (your working timings)
...
// Templates (39 bytes)
// modify Byte[26] tempX2, Byte[28] fan, then recompute Byte[19]/Byte[38]
...
struct ACState {
  bool powerOn;
  int tempX2;     // temperature * 2 (0.5C)
  ACMode mode;
  uint8_t fanHex; // Byte[28]
};

static ACState ac3F = { false, 55, MODE_HEAT, 0xA0 };

// fan map (verified by you)
...
static void addByteToBuffer(uint8_t data) {
  for (int i = 0; i < 8; i++) {
    ...
    rawBuffer[rawIdx++] = ((data >> i) & 1) ? ONE_SPACE : ZERO_SPACE; // LSB-first
  }
}
...
static void clampTemp(ACState& st) {
  if (st.tempX2 < 36) st.tempX2 = 36; // 18.0
  if (st.tempX2 > 64) st.tempX2 = 64; // 32.0
}
...
static void light_on() {
  sendFrames(ON_f1, sizeof(ON_f1)/2, ON_f2, sizeof(ON_f2)/2, ON_f3, sizeof(ON_f3)/2, kRepeatCount);
}
static void light_night() {
  sendFrames(NIGHT_f1, sizeof(NIGHT_f1)/2, NIGHT_f2, sizeof(NIGHT_f2)/2, nullptr, 0, kRepeatCount);
}
static void light_off() {
  sendFrames(OFF_f1, sizeof(OFF_f1)/2, OFF_f2, sizeof(OFF_f2)/2, nullptr, 0, kRepeatCount);
}
...
static void handleAC3State() {
  ...
  if (offDeadlineMs != 0) {
    uint64_t now = millis();
    if (offDeadlineMs > now) left = String((offDeadlineMs - now) / 1000) + "s";
    else left = "0s";
  }
...
static void handleAC3Api() {
  if (server.hasArg("mode")) {
    ...
    if (m == "heat") {
      ac3F.mode = MODE_HEAT;
      ac3F.tempX2 = 55; // 默认 27.5
    } else if (m == "cool") {
      ac3F.mode = MODE_COOL;
      ac3F.tempX2 = 48; // 默认 24.0
    }
    ac3F.powerOn = true;
    if (ac3F.fanHex == 0) ac3F.fanHex = FAN_AUTO;
    clampTemp(ac3F);
    buildAndSendRun(ac3F);
    server.send(200, "text/plain; charset=utf-8", "已发送：模式");
    return;
  }
  ...
  if (server.hasArg("tempStep")) {
    ...
    server.send(200, "text/plain; charset=utf-8", "已发送：温度");
    return;
  }
  ...
  if (server.hasArg("fan")) {
    ...
    if (ac3F.powerOn) buildAndSendRun(ac3F);
    server.send(200, "text/plain; charset=utf-8", "已发送：风量");
    return;
  }
  ...
  if (server.hasArg("power")) {
    ...
    if (p == 0) {
      sendOffNow();
      server.send(200, "text/plain; charset=utf-8", "已发送：关机");
      return;
    }
  }
  ...
  if (server.hasArg("timerMin")) {
    ...
    if (mins <= 0) {
      server.send(400, "text/plain; charset=utf-8", "分钟数不合法");
      return;
    }
    offDeadlineMs = (uint64_t)millis() + (uint64_t)mins * 60ULL * 1000ULL;
    server.send(200, "text/plain; charset=utf-8", "已设置：定时关机");
    return;
  }

  if (server.hasArg("timerCancel")) {
    offDeadlineMs = 0;
    server.send(200, "text/plain; charset=utf-8", "已取消：定时关机");
    return;
  }
...
static void handleLightApi() {
  ...
  if (c == "on") {
    light_on();
    server.send(200, "text/plain; charset=utf-8", "已发送：全灯");
  } else if (c == "night") {
    light_night();
    server.send(200, "text/plain; charset=utf-8", "已发送：常夜灯");
  } else if (c == "off") {
    light_off();
    server.send(200, "text/plain; charset=utf-8", "已发送：消灯");
  } else {
...
void setup() {
  ...
  Serial.print("Static Connecting");
